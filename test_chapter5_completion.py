...
def test_chapter5_completion():
    """
    Test the Chapter 5 completion functionality and the achievements.
    """
    # Create a database session
    db = SessionLocal()
    
    try:
        print("Starting Chapter 5 completion test...")
        
        # Create a test user
        test_user = schemas.UserCreate(
            email="test_user_ch5@example.com",
            username="test_user_ch5",
        ...
        # Get all quizzes for Chapter 5
        quizzes = crud.get_quizzes_by_chapter(db, chapter.id)
        if not quizzes:
            print("Error: No quizzes found for Chapter 5. Make sure the database is seeded.")
            return False
        ...
        # Check if user has earned all Chapter 5 achievements
        user_achievements = crud.get_user_achievements(db, user.id)
        all_achievements_earned = True
        
        # List of expected Chapter 5 achievements
        expected_achievements = ["Allocation Architect", "Diversification Dynamo", "Rebalancing Ruler", "Policy Pro"]
...
if __name__ == "__main__":
    success = test_chapter5_completion()
